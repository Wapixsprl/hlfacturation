...
export async function POST(request: NextRequest) {
  ...
  const docType = searchParams.get('type') || 'documents'
  ...
  if (!ALLOWED_TYPES.includes(file.type)) {
    return NextResponse.json(
      { error: 'Format non supporte. Utilisez PDF, PNG, JPG ou WebP.' },
      { status: 400 }
    )
  }
  ...
  const safeName = file.name
    ...
    .substring(0, 50)
  const filePath = `${utilisateur.entreprise_id}/documents/${docType}/${timestamp}_${safeName}.${ext}`
